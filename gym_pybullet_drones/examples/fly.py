...
def run(
        ...
        pyb_freq=DEFAULT_PYBULLET_FREQ_HZ,
    ...
    AGGR_PHY_STEPS = int(pyb_freq/ctrl_freq) if aggregate else 1
    ...
    logger = Logger(logging_freq_hz=int(pyb_freq/AGGR_PHY_STEPS),
                    num_drones=num_drones,
    ...
    #### Run the simulation ####################################
    CTRL_EVERY_N_STEPS = int(np.floor(env.PYB_FREQ/ctrl_freq))
    # action = {str(i): np.array([0,0,0,0]) for i in range(num_drones)}
    action = np.zeros((num_drones, 4))
    START = time.time()
    for i in range(0, int(duration_sec*env.PYB_FREQ), AGGR_PHY_STEPS):
        ...
        print(outputs)  # See what exactly is being returned
        obs, reward, done, info = outputs[:4]  # Safely unpack the first four elements
        ...
        if i%CTRL_EVERY_N_STEPS == 0:
            ...
            for j in range(num_drones):
                action[str(j)], _, _ = ctrl[j].computeControlFromState(control_timestep=CTRL_EVERY_N_STEPS,
...
if __name__ == "__main__":
    ...
    parser.add_argument('--pyb_freq',           default=DEFAULT_PYBULLET_FREQ_HZ,        type=int,           help='Simulation frequency in Hz (default: 240)', metavar='')
    parser.add_argument('--ctrl_freq',          default=DEFAULT_CONTROL_FREQ_HZ,         type=int,           help='Control frequency in Hz (default: 48)', metavar='')
